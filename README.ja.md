...
テンプレートから回転方向場群を事前計算し、入力フレームでは縮小画像上の大域探索と
原寸画像上の局所探索を順に行います。図は処理関係を示す模式図であり、固定スケール・
最良一致1件という現在の適用範囲を表しています。
...
粗段は全位置と粗角度を評価して探索範囲を覆い、精密段は上位`K`姿勢の周囲だけに計算を
集中します。この探索は高速化のためのヒューリスティックであり、狭い大域最大を必ず保持
する保証はありません。
