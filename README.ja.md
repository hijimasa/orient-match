...
構築時に一度だけ作ります。各フレームでは方向場に変換し、縮小画像上の大域探索と原寸画像上の
局所探索を順に行います。図は処理関係を示す模式図であり、固定スケール・最良一致1件という
現在の適用範囲を表しています。
...
スコアは `(x, y, θ)` 上の3次元空間に広がります。粗段は縮小画像でその全体を粗く走査し、
精密段は上位`K`姿勢の周囲だけを原寸で詳しく見ます。これにより、原寸での総当たり探索に比べて
計算量はごく一部で済みます。ただし高速化のためのヒューリスティックであり、狭い大域最大を
必ず保持する保証はありません。
