...
from data import (FINE_OFFSETS, OUTER_FINE_OFFSETS, REFINE_TOP_K, SCAN_STRIDE,
                  coarse_to_fine, cost_estimate)
from svgutil import *

D = coarse_to_fine()
angles, maps, peaks, order = D["angles"], D["maps"], D["peaks"], D["order"]
scanned, bases = D["scanned"], D["bases"]
...
g = Svg(W, H, "OrientMatch coarse-to-fine pose search",
        "The global stage scores every position on a half-resolution image, but only at every "
        "fourth coarse angle. The best separated places are then refined: first over the angles the "
        "scan stepped across, then at full resolution over a small position window and 1 degree angles.")
g.text(W/2, 50, "Why the search is staged", 28, INK, "700", "middle")
g.text(W/2, 80, "the score is a 3-D volume over (x, y, θ) — sample it coarsely in every axis, then look "
       "closely only where it is promising", 15, MUTED, "400", "middle")

# ------------------------------------------------ stage 1
y0 = 104
card(g, 40, y0, 640, 500, "STAGE 1   global — all positions × every 4th angle, on the 0.5× image",
...
cell = 176.0/mw
for i, a in enumerate((0.0, 36.0, 60.0)):
    x = 76 + i*192
    g.text(x + 88, y0+72, f"θ = {a:.0f}°", 13, "#8a4a04" if a == 60.0 else MUTED, "700", "middle")
    ...
    g.rect(x, y0+82, mw*cell, mh*cell, rx=3, fill="none",
           stroke="#d9a066" if a == 60.0 else "#c3cbd8", stroke_width=2 if a == 60.0 else 1)
...
g.text(360, cby+34, f"one complete translation score map per scanned angle — {len(scanned)} of the "
       f"{len(angles)} in the bank", 13, MUTED, "400", "middle")
...
for i in scanned:
    tx = px0 + angles[i]/360.0*pw
    g.add(f'<path d="M{tx:.1f} {py0+ph} v7" stroke="#b85c00" stroke-width="1.6"/>')
for a in (0, 90, 180, 270, 360):
    g.text(px0 + a/360.0*pw, py0+ph+26, f"{a}°", 11, "#9aa4b5", "400", "middle")
g.text(px0, py0-10, "best score found at each coarse angle", 12.5, MUTED, "600")
g.text(px0+pw, py0-10, "| = scanned    ○ = kept for refinement", 12, "#b85c00", "600", "end")
bx = px0 + 330/360.0*pw
...
g.text(90, y0+468, "the curve is smooth in θ, so a scan this sparse still peaks on the object",
       13, MUTED)
...
g.text(820, 306, f"up to K = {REFINE_TOP_K} places", 12.5, MUTED, "400", "middle")
...
g.text(820, 452, "⋮" if len(order) > 4 else f"{len(order)} here", 12.5, MUTED, "700", "middle")
g.text(820, 486, "kept half a canvas apart, so", 11.5, MUTED, "400", "middle")
g.text(820, 502, "each one is a different place", 11.5, MUTED, "400", "middle")
arrow(g, 900, 330, 940, 330, "#6f7c90", 2.5, 10)

# ------------------------------------------------ stage 2
card(g, 952, y0, 408, 500, "STAGE 2   local — angles, then full resolution",
...
g.text(1180, y0+134, "around each refined peak", 12.5, MUTED)
g.text(1180, y0+164, "the angles the global scan", 12.5, MUTED)
g.text(1180, y0+184, "stepped over are retried first,", 12.5, MUTED)
g.text(1180, y0+204, "on the 0.5× image", 12.5, MUTED)

# fine angle bars
by0 = y0+256
g.text(1005, by0-12, "then the neighbouring 1° angles", 12.5, MUTED, "600")
offs = [-3, -2, -1, 0, 1, 2, 3]
best_offset = max(offs, key=lambda o: fine[(0, o)][0])
bw, gap = 34, 12
BASE = 0.76
...
_bo = best_offset
_bs, _bmap, _ba = fine[(0, _bo)]
_by, _bx = np.unravel_index(_bmap.argmax(), _bmap.shape)
_rx, _ry, _, _ = rois[0]
g.text(1062, by0+216, "center (%.1f, %.1f) · θ %.0f° · %.2f"
       % (_rx + _bx + (size - 1) / 2.0, _ry + _by + (size - 1) / 2.0, _ba, _bs),
       13, "#145c35", "700", font=MONO)
...
fw_, fh_ = fine[(0, 0)][1].shape[1], fine[(0, 0)][1].shape[0]
rows = [("full-resolution exhaustive search, 1° steps", ex, "#b6bfcd", f"{ex/1e9:.0f} G"),
        (f"global scan (0.5× image, every {SCAN_STRIDE}th of the 3° angles)", c1, ORANGE,
         f"{c1/1e9:.1f} G"),
        (f"fine stage ({len(order)} places × {len(FINE_OFFSETS)} angles + "
         f"{len(OUTER_FINE_OFFSETS)}, {fw_}×{fh_} offsets)", c2, BLUE, f"{c2/1e9:.2f} G")]
